...
class VLLMProvider(BaseProvider):
    ...
    def _get_headers(self) -> dict[str, str]:
        return {"Authorization": f"Bearer {self._config.api_key or ''}"} if self._config.api_key else {}
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        model_name = model or self._config.default_model or "openai/gpt-3.5-turbo"
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            models = await litellm.amodel_list(
                ...
                api_base=self._config.base_url or "http://localhost:8000/v1",
            ...
            return [
                ModelInfo(
                    model_name=self._config.default_model or "openai/gpt-3.5-turbo",
                    provider=self.name,
                )
            ]
