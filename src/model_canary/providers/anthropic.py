...
class AnthropicProvider(BaseProvider):
    ...
    def _get_headers(self) -> dict[str, str]:
        ...
        if self._config.organization:
            headers["x-api-key"] = self._config.organization
        return headers

    def _get_api_url(self) -> str:
        return self._config.base_url or "https://api.anthropic.com/v1"
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        model_name = model or self._config.default_model or "claude-3-opus-20240229"
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            models = await litellm.amodel_list(
                model=f"anthropic/{self._config.api_key or ''}",
            ...
            return [
                ModelInfo(
                    model_name=self._config.default_model or "claude-3-opus-20240229",
                    provider=self.name,
                )
            ]
