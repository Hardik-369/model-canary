...
class CohereProvider(BaseProvider):
    ...
    def _get_api_url(self) -> str:
        return self._config.base_url or "https://api.cohere.ai/v1"
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        model_name = model or self._config.default_model or "cohere/command-r-plus"
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            models = await litellm.amodel_list(
                model=f"cohere/{self._config.api_key or ''}",
            ...
            return [
                ModelInfo(
                    model_name=self._config.default_model or "cohere/command-r-plus",
                    provider=self.name,
                )
            ]
