...
class AzureOpenAIProvider(BaseProvider):
    ...
    def _get_headers(self) -> dict[str, str]:
        return {"api-key": self._config.api_key or ""}

    def _get_api_url(self) -> str:
        return self._config.base_url or "https://YOUR_RESOURCE.openai.azure.com"
    ...
    def _get_litellm_params(self) -> dict[str, Any]:
        ...
        if hasattr(self._config, 'extra') and self._config.extra:
            api_version = self._config.extra.get("api_version")
            if api_version:
                params["api_version"] = api_version
            for k, v in self._config.extra.items():
                if k not in ("api_version",):
                    params[k] = v
        return params
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        model_name = model or self._config.default_model or "azure/gpt-4o"
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            models = await litellm.amodel_list(
                model=f"azure/{self._config.api_key or ''}",
            ...
            return [
                ModelInfo(
                    model_name=self._config.default_model or "azure/gpt-4o",
                    provider=self.name,
                )
            ]
