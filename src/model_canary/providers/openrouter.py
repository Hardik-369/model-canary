...
class OpenRouterProvider(BaseProvider):
    ...
    def _get_headers(self) -> dict[str, str]:
        ...
        if hasattr(self._config, 'extra') and self._config.extra:
            if "referer" in self._config.extra:
                headers["HTTP-Referer"] = self._config.extra["referer"]
            if "title" in self._config.extra:
                headers["X-Title"] = self._config.extra["title"]
        return headers

    def _get_api_url(self) -> str:
        return self._config.base_url or "https://openrouter.ai/api/v1"
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        model_name = model or self._config.default_model or "openrouter/openai/gpt-4o"
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            models = await litellm.amodel_list(
                model=f"openrouter/{self._config.api_key or ''}",
            ...
            return [
                ModelInfo(
                    model_name=self._config.default_model or "openrouter/openai/gpt-4o",
                    provider=self.name,
                )
            ]
