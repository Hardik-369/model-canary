...
class GrokProvider(BaseProvider):
    ...
    def _get_api_url(self) -> str:
        return self._config.base_url or "https://api.x.ai/v1"
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        model_name = model or self._config.default_model or "grok/grok-2"
    ...
    async def list_models(self) -> list[ModelInfo]:
        try:
            models = await litellm.amodel_list(
                model=f"grok/{self._config.api_key or ''}",
                custom_llm_provider="grok",
            )
            return [
                ModelInfo(
                    model_name=m.get("id", ""),
            ...
            return [
                ModelInfo(
                    model_name=self._config.default_model or "grok/grok-2",
                    provider=self.name,
                )
            ]
